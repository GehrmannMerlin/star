...
这是一个 **Agent Crawlee 专项实现诊断、修复顾问 Skill**，用于对爬虫项目的 Crawlee 实现行为进行证据驱动的分类诊断，输出可追溯问题证据、根因或待验证假设、影响范围、修复方向（Crawlee 实现建议）和测试建议。它**不是**项目爬虫运行时的替代品；不是修复器、规划器、记忆服务、RAG、代码索引、上下文框架或爬虫运行组件。它引用锁定版本（apify/crawlee v3.17.0）的 API 与配置语义，将领域 Skill 已确认的方向转换为当前版本实现建议，并识别"配置合法但运行行为不符合预期"的问题。

## 触发

用户要求检查 Crawlee 专项实现相关设计或故障，或已经确认的问题涉及 Crawler 选择、`RequestQueue`、`Dataset`、`AutoscaledPool`、`SessionPool`、代理、生命周期或配置时触发。处理 Stellaris 问题时，先由 `stellaris-crawler-context` 注入项目事实与已批准约束，再交给本 Skill。
...
4. 已确认根因与方向（`crawler-triage-incidents` 分诊结论，适用时）、项目目标/约束/现状、相关 Crawlee 代码/配置/依赖版本/日志/最小复现证据。

详细诊断流程与实现转换规则见 `references/crawlee-workflow.md`；输出契约与交接见 `references/output-contract.md`；知识视图见 `docs/superpowers/knowledge/skill-views/crawler-use-crawlee.md`；证据卡见 `docs/superpowers/knowledge/evidence-cards/crawlee.md`。

## 非协商门禁

1. 识别为 Crawlee 实现诊断任务（需要基于证据的分类）。
2. 处理 Stellaris 问题时，先读取 `stellaris-crawler-context` 项目上下文包路径（主决策日志 §38）。
3. 明确诊断聚焦点（Crawler 选择与生命周期 / `RequestQueue` 与请求调度 / `Dataset` 与数据存储 / `AutoscaledPool` 与资源调节 / `SessionPool` 与会话管理 / 代理与配置），聚焦该点，不做无边界全栈诊断。
4. 按匹配版本证据卡分类；引用锁定版本（apify/crawlee v3.17.0）的 API 与配置语义；绝不因证据不足编造根因；**绝不以"配置合法"误判为"运行行为正确"**。
5. 外部条件（运行时环境不可用/外部服务受限）只能给合规降级/等待/终止结论；绝不伪装成已修复。
6. 呈现诊断记录并等待用户批准；批准前不交给 `crawler-writing-plans-bridge`。
7. 绝不替代项目爬虫运行时；绝不重复跨框架 Frontier/HTTP/浏览器/队列方法论（路由到对应领域 Skill）；绝不生成 `writing-plans` 或越过批准边界；绝不安装、构建或运行未经批准的项目或第三方代码；绝不保存密钥、Cookie、认证请求头、代理凭据。
8. 输出可追溯产物并更新本地进度总账后才继续。

## 工作流路由

- 诊断流程、问题分类规则、实现转换规则、证据卡映射：`references/crawlee-workflow.md`。
- 诊断记录输出契约、交接契约、修订保留：`references/output-contract.md`。
- 知识视图与证据卡：`docs/superpowers/knowledge/skill-views/crawler-use-crawlee.md`、`docs/superpowers/knowledge/evidence-cards/crawlee.md`。
...
- 不替代项目爬虫运行时。
- 不重复跨框架 Frontier/HTTP/浏览器/队列方法论。
