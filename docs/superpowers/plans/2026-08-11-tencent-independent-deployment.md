...
迁移 `down` 删除索引与复合约束、恢复单列幂等约束、再删除 `owner_user_id`。在 `migration-provider.ts` 注册为第三项，并将 `TaskRunTable` 增加 `owner_user_id: string`。
...
验证 `http://127.0.0.1:3218/` 返回专用前端，资源路径构建正确；`http://127.0.0.1:3217/health` 返回 `200`。从 PostgreSQL 查询 `task_run` 数量必须为 `0`，迁移最新项必须是 `003-task-owner`，`owner_user_id` 必须为 `NOT NULL`。
