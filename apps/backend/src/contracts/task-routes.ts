...
export async function registerTaskRoutes(
  ...
  app.get<{ Querystring: import("@stellaris/contracts").TaskListParams }>("/api/tasks", async (req) => {
    const limit = Math.min(Math.max(req.query.limit ?? 20, 1), 100);
    const offset = Math.max(req.query.offset ?? 0, 0);
    const status = req.query.status;
    const mode = req.query.mode;
    const rows = await repos.taskRun.listRecent({ limit, offset, ...(status ? { status } : {}), ...(mode ? { mode } : {}) });
    const total = await repos.taskRun.count({ ...(status ? { status } : {}), ...(mode ? { mode } : {}) });
  ...
  app.get<{ Params: { id: string } }>("/api/tasks/:id", async (req) => {
    const task = await repos.taskRun.findById(req.params.id);
  ...
  // 结果。
  app.get<{ Params: { id: string } }>("/api/tasks/:id/results", async (req) => {
  ...
  app.post<{ Params: { id: string } }>("/api/tasks/:id/duplicate", async (req) => {
    const r = await duplicateTask(req.params.id, controlDeps);
    return toControlResponse(r);
  });
}
