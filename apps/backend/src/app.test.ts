...
function injectA(targetApp: FastifyInstance, options: InjectOptions) {
  return injectAs(targetApp, USER_A, options);
}
...
async function waitForTaskCompletion(taskId: string, timeoutMs = 15_000): Promise<void> {
  ...
  while (Date.now() < deadline) {
    const detail = await injectA(app, { method: "GET", url: `/api/tasks/${taskId}` });
...
describe("任务 API 与 SSE", () => {
  it("创建任务返回 created 且任务状态为已完成", async () => {
    const res = await injectA(app, { method: "POST", url: "/api/tasks", payload: validCreate });
    expect(res.statusCode).toBe(200);
    expect(res.json().idempotencyResult).toBe("created");
    const id = res.json().task.id;
    await waitForTaskCompletion(id);
    const detail = await injectA(app, { method: "GET", url: `/api/tasks/${id}` });
    expect(detail.statusCode).toBe(200);
    expect(detail.json().task.statusZh).toBe("已完成");
  });

  it("同幂等键重复创建返回 replayed 且同一任务 id", async () => {
    const a = await injectA(app, { method: "POST", url: "/api/tasks", payload: validCreate });
    const b = await injectA(app, { method: "POST", url: "/api/tasks", payload: validCreate });
  ...
  it("结果含两槽位且 PRIMARY_1 的 positionUrl 已被 Reviewer 复核", async () => {
    const id = (await injectA(app, { method: "POST", url: "/api/tasks", payload: validCreate })).json().task.id;
    await waitForTaskCompletion(id);
    const results = (await injectA(app, { method: "GET", url: `/api/tasks/${id}/results` })).json();
...
describe("可信身份与逐任务授权", () => {
  it("缺少可信身份时拒绝创建、列表与会话探针", async () => {
    const create = await app.inject({
      method: "POST",
      url: "/api/tasks",
      payload: { ...validCreate, clientIdempotencyKey: `unauthenticated-${Date.now()}` },
    });
    expect(create.statusCode).toBe(401);
    expect(create.json()).toEqual({ error: "UNAUTHENTICATED" });

    const list = await app.inject({ method: "GET", url: "/api/tasks" });
    expect(list.statusCode).toBe(401);
    expect(list.json()).toEqual({ error: "UNAUTHENTICATED" });

    const session = await app.inject({ method: "GET", url: "/api/session" });
    expect(session.statusCode).toBe(401);
    expect(session.json()).toEqual({ error: "UNAUTHENTICATED" });
  });

  it("会话探针返回可信用户", async () => {
    const session = await injectA(app, { method: "GET", url: "/api/session" });
    expect(session.statusCode).toBe(200);
    expect(session.json()).toEqual({ authenticated: true, userId: "user-a" });
  });

  it("另一用户对任务关联端点一律得到 404", async () => {
    const created = await injectA(app, {
      method: "POST",
      url: "/api/tasks",
      payload: { ...validCreate, clientIdempotencyKey: `owner-boundary-${Date.now()}` },
    });
    const taskId = created.json().task.id as string;
    await waitForTaskCompletion(taskId);

    const results = (await injectA(app, {
      method: "GET",
      url: `/api/tasks/${taskId}/results`,
    })).json() as Array<{ id: string }>;
    const resultRowId = results[0]!.id;

    const ownerExport = await injectA(app, { method: "GET", url: `/api/tasks/${taskId}/export` });
    expect(ownerExport.statusCode).toBe(200);

    const crossOwnerRequests: InjectOptions[] = [
      { method: "GET", url: `/api/tasks/${taskId}` },
      { method: "GET", url: `/api/tasks/${taskId}/results` },
      { method: "GET", url: `/api/tasks/${taskId}/evidence/${resultRowId}` },
      { method: "GET", url: `/api/tasks/${taskId}/export` },
      { method: "POST", url: `/api/tasks/${taskId}/pause` },
      { method: "POST", url: `/api/tasks/${taskId}/resume` },
      { method: "POST", url: `/api/tasks/${taskId}/cancel` },
      { method: "POST", url: `/api/tasks/${taskId}/duplicate` },
    ];
    for (const request of crossOwnerRequests) {
      const response = await injectAs(app, USER_B, request);
      expect(response.statusCode, `${request.method} ${request.url}`).toBe(404);
      expect(response.json()).toEqual({ error: "TASK_NOT_FOUND" });
    }

    const sseController = new AbortController();
    const sse = await fetch(`http://127.0.0.1:${serverPort}/api/tasks/${taskId}/events`, {
      headers: USER_B,
  ...
  it("创建幂等性、列表内容与 total 均按用户隔离", async () => {
    const idempotencyKey = `owner-list-${Date.now()}`;
    const a = await injectA(app, {
      method: "POST",
      url: "/api/tasks",
      payload: { ...validCreate, clientIdempotencyKey: idempotencyKey },
    });
    const b = await injectAs(app, USER_B, {
      method: "POST",
      url: "/api/tasks",
      payload: { ...validCreate, clientIdempotencyKey: idempotencyKey },
    });
    const aId = a.json().task.id as string;
    const bId = b.json().task.id as string;
    expect(bId).not.toBe(aId);

    const aList = (await injectA(app, { method: "GET", url: "/api/tasks?limit=100" })).json();
    const bList = (await injectAs(app, USER_B, { method: "GET", url: "/api/tasks?limit=100" })).json();
    expect(aList.tasks.some((task: { id: string }) => task.id === aId)).toBe(true);
    expect(aList.tasks.some((task: { id: string }) => task.id === bId)).toBe(false);
    expect(aList.total).toBe(aList.tasks.length);
    expect(bList.tasks.map((task: { id: string }) => task.id)).toEqual([bId]);
...
describe("完整机构模式（FULL_INSTITUTION）", () => {
  ...
  it("FULL_INSTITUTION 创建任务 → 机构发现 → 多机构结果", async () => {
    ...
    const created = (await injectA(app2, { method: "POST", url: "/api/tasks", payload })).json();
    ...
    let statusZh = "";
    while (Date.now() < deadline) {
      const detail = await injectA(app2, { method: "GET", url: `/api/tasks/${id}` });
    ...
    const detail = (await injectA(app2, { method: "GET", url: `/api/tasks/${id}` })).json();
    expect(detail.task.statusZh).toBe("已完成");

    // 结果：每机构两槽位。
    const results = (await injectA(app2, { method: "GET", url: `/api/tasks/${id}/results` })).json();
...
describe("多行政区模式（regionCodes）", () => {
  ...
  it("POST /api/tasks regionCodes → 多行政区冻结 → 结果", async () => {
    ...
    const created = (await injectA(app3, { method: "POST", url: "/api/tasks", payload })).json();
    expect(created.idempotencyResult).toBe("created");
    const id = created.task.id;
    ...
    let statusZh = "";
    while (Date.now() < deadline) {
      const detail = await injectA(app3, { method: "GET", url: `/api/tasks/${id}` });
      statusZh = detail.json()?.task?.statusZh ?? "";
      if (statusZh !== "待开始" && statusZh !== "正在准备" && statusZh !== "正在抓取") {
        break;
      }
    ...
    const results = (await injectA(app3, { method: "GET", url: `/api/tasks/${id}/results` })).json() as Array<{ regionCode: string }>;
...
describe("任务控制 API（暂停/继续/取消/复制）", () => {
  ...
  it("POST /api/tasks/:id/pause → 任务 PAUSED；resume → 任务最终完成", async () => {
    ...
    const created = (await injectA(app4, { method: "POST", url: "/api/tasks", payload })).json();
    const id = created.task.id;
    ...
    while (Date.now() < deadline) {
      status = (await injectA(app4, { method: "GET", url: `/api/tasks/${id}` })).json()?.task?.status ?? "";
      if (status === "CRAWLING") break;
      await new Promise((r) => setTimeout(r, 100));
    }
    expect(status).toBe("CRAWLING");

    const paused = await injectA(app4, { method: "POST", url: `/api/tasks/${id}/pause` });
    expect(paused.statusCode).toBe(200);
    expect(paused.json().ok).toBe(true);
    expect(paused.json().task.status).toBe("PAUSED");

    const resumed = await injectA(app4, { method: "POST", url: `/api/tasks/${id}/resume` });
    ...
    let statusZh = "";
    while (Date.now() < d2) {
      const detail = await injectA(app4, { method: "GET", url: `/api/tasks/${id}` });
      statusZh = detail.json()?.task?.statusZh ?? "";
      if (statusZh !== "待开始" && statusZh !== "正在准备" && statusZh !== "正在抓取") {
        break;
      }
  ...
  it("POST /api/tasks/:id/cancel → CANCELLED；duplicate → 新任务；终态不可再控制", async () => {
    ...
    const cancelled = await injectA(app4, { method: "POST", url: `/api/tasks/${id}/cancel` });
    expect(cancelled.statusCode).toBe(200);
    expect(cancelled.json().ok).toBe(true);
    expect(cancelled.json().task.status).toBe("CANCELLED");

    // 已取消任务不能继续。
    const resumeCancelled = await injectA(app4, { method: "POST", url: `/api/tasks/${id}/resume` });
    expect(resumeCancelled.json().ok).toBe(false);

    // 可复制为新任务。
    const dup = await injectA(app4, { method: "POST", url: `/api/tasks/${id}/duplicate` });
  ...
  it("任务详情含 controlable 字段", async () => {
    const created = (await injectA(app, { method: "POST", url: "/api/tasks", payload: validCreate })).json();
    const id = created.task.id;
    const detail = await injectA(app, { method: "GET", url: `/api/tasks/${id}` });
...
describe("P3 任务列表端点（GET /api/tasks）", () => {
  it("默认返回最近任务（降序）且含 total", async () => {
    ...
    const res = await injectA(app, { method: "GET", url: "/api/tasks" });
    expect(res.statusCode).toBe(200);
    const body = res.json();
  ...
  it("limit/offset 分页生效", async () => {
    const page1 = (await injectA(app, { method: "GET", url: "/api/tasks?limit=2&offset=0" })).json();
    const page2 = (await injectA(app, { method: "GET", url: "/api/tasks?limit=2&offset=2" })).json();
  ...
  it("status/mode 筛选生效且 total 一致", async () => {
    ...
    const res = await injectA(app, { method: "GET", url: "/api/tasks?status=COMPLETED&mode=TARGETED" });
    const body = res.json();
  ...
  it("limit 上限 100，非法参数被钳制", async () => {
    const res = await injectA(app, { method: "GET", url: "/api/tasks?limit=9999" });
    expect(res.statusCode).toBe(200);
    const body = res.json();
...
describe("R-37 缺入口自动补全容错（真实安徽适配器）", () => {
  ...
  async function createTargetedEntryUrl(name: string): Promise<string | null> {
    ...
    const res = await injectA(app5, { method: "POST", url: "/api/tasks", payload });
    expect(res.statusCode).toBe(200);
    const id = res.json().task.id;
    const detail = (await injectA(app5, { method: "GET", url: `/api/tasks/${id}` })).json();
  ...
  it("未适配机构（省民宗委）→ 不误配，入口保持空且任务正常终态（R-37 空入口修复）", async () => {
    ...
    const created = (await injectA(app5, { method: "POST", url: "/api/tasks", payload })).json();
    const id = created.task.id;
    const deadline = Date.now() + 15_000;
    let statusZh = "";
    while (Date.now() < deadline) {
      const detail = await injectA(app5, { method: "GET", url: `/api/tasks/${id}` });
      statusZh = detail.json()?.task?.statusZh ?? "";
    ...
    const results = (await injectA(app5, { method: "GET", url: `/api/tasks/${id}/results` })).json();
