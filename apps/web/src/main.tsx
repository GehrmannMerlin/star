...
const realApi: ApiClient = {
  ...
  async getResults(id: string) {
    const res = await fetch(apiUrl(`tasks/${id}/results`));
    return (await res.json()) as ResultRowView[];
  },
  async getEvidence(id: string, resultRowId: string) {
    const res = await fetch(apiUrl(`tasks/${id}/evidence/${resultRowId}`));
    return (await res.json()) as EvidenceDetail;
  },
  async downloadExport(id: string) {
    return { url: apiUrl(`tasks/${id}/export`) };
  },
  openEvents(id: string) {
    return new EventSource(apiUrl(`tasks/${id}/events`));
  },
  ...
  async listProvinces() {
    const res = await fetch(apiUrl("regions/provinces"));
    return (await res.json()) as RegionNode[];
  },
  async listChildren(parent: string) {
    const res = await fetch(apiUrl(`regions/children?parent=${encodeURIComponent(parent)}`));
    return (await res.json()) as RegionNode[];
  },
  async validateRegions(codes: string[]) {
    const res = await fetch(apiUrl("regions/validate"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async expandRegions(codes: string[], level: "COUNTY" | "TOWN_STREET") {
    const res = await fetch(apiUrl("regions/expand"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
